...
# Amazon Nova by default because it needs no use-case submission; Anthropic
# models work identically through the Converse API once "model use case
# details" are submitted for the account in the Bedrock console.
...
def _metric(name, value=1.0, **dimensions):
    ...
    Metrics are observability, not the record. The verdict is already durable
    in DynamoDB by the time this runs, so letting a metrics outage fail the
    message would redeliver work that is already complete — and, with a
    deterministic key, rewrite the same item forever. Log the failure by name
    ...
    try:
        ...
        print(json.dumps({"warning": "metric_emit_failed", "metric": name,
...
def propose(company, cik, fiscal_year):
    ...
    Uses Bedrock's Converse API rather than invoke_model: one request shape for
    every provider, so swapping Nova for Anthropic is an environment variable
    ...
    try:
        response = bedrock.converse(
            ...
            messages=[{
                "role": "user",
                "content": [{"text": PROMPT.format(
                    company=company, cik=cik, fiscal_year=fiscal_year)}],
            }],
