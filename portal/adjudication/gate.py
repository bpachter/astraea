"""Client for the Astraea .NET reconciliation gate.
...
def call_gate(path, body):
    ...
    url = f"{settings.ASTRAEA_GATE_URL}{path}"
    try:
        response = requests.post(url, json=body, timeout=10)
        response.raise_for_status()
    except requests.RequestException as exc:
        raise GateUnreachable(
            f"Reconciliation gate unreachable at {url} — is Astraea.Gate running? ({exc})"
        ) from exc
