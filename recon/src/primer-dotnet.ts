...
// Every chapter anchors to code in THIS repo (Astraea.Gate is the reader's own
...
export const DOTNET_TRACK_CREDIT =
  "Anchored to gate/Astraea.Gate in this repository — a working ASP.NET Core 8 " +
...
export const DOTNET_CHAPTERS: PrimerChapter[] = [
  {
    ...
    sections: [
      {
        paragraphs: [
          "The vocabulary stack, bottom-up: C# is the language; .NET is the platform (runtime + libraries + SDK); the CLR is the virtual machine that JIT-compiles your IL to native code ('managed code' means the CLR manages memory and safety); the BCL is the standard library; ASP.NET Core is the web framework on top. Say '.NET' today and you mean the modern cross-platform one — the Windows-only lineage is called '.NET Framework' (4.8, legacy), and the transition era was branded '.NET Core'. If a teammate says 'Core', they're distinguishing from Framework.",
          "Release cadence is annual, alternating LTS and STS: .NET 6 and 8 were LTS, 10 is the current LTS (late 2025). Astraea targets net8.0 — that string in the .csproj is the TFM (target framework moniker) — and 'we're planning the 8-to-10 bump' is a normal, low-drama sentence on any team. NuGet is pip/npm; the .csproj is pyproject.toml/package.json (PackageReference entries are your dependencies); a .sln groups projects the way a monorepo workspace does. `dotnet restore / build / test / run / publish` is the CLI you already used to build this repo's gate.",
    ...
    terms: [
      ...
      { term: "TFM (net8.0)", def: "Target framework moniker in the .csproj — which platform version you compile against. Astraea.Gate: net8.0, the LTS it was built on." },
    ...
    checks: [
      { q: "A teammate asks 'Framework or Core?' — what are they really asking?", a: "Whether the codebase is legacy Windows-only .NET Framework 4.x or modern cross-platform .NET. The answer shapes everything: deployment targets, library availability, and how painful the migration story is." },
    ...
    sections: [
      {
        paragraphs: [
          ...
          "The bridge sentence for your own story: Astraea's portal runs Django ORM over SQLite with the same env-driven swap-to-Postgres posture EF Core teams use — you speak both ORMs' dialects, and the gate's typed rules would sit in front of either.",
        ],
      },
    ],
    terms: [
      ...
      { term: "Eager loading / N+1 / AsNoTracking", def: "Include() to join up front; the query-per-row trap; tracking off for read-only speed — the three EF phrases reviews reward." },
  ...
  },
  {
    ...
    terms: [
      ...
      { term: "ConfigMap / Secret", def: "Injected configuration vs injected sensitive configuration — the K8s slot where env-driven settings (like Astraea's DJANGO_*) land." },
    ],
    checks: [
    ...
    sections: [
      {
        paragraphs: [
          ...
          "Your bridge is genuinely strong, so say it plainly: you've shipped a tested ASP.NET Core 8 service this quarter (typed domain records, minimal APIs, DI, env-driven config, containerized, 22 tests); your daily drivers are Python and TypeScript, and the concepts transfer — Django ORM to EF Core, asyncio to Task, Celery-shaped thinking to BackgroundService and queues. 'It's been years since my C# was daily, so my recent .NET is deliberate re-sharpening — here's the repo' beats any bluff, and reviewers respect an engineer who knows exactly where their rust is.",
